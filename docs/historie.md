...
|---|---|---|
| 2026-08-22 | **Sprint 40** — vier neue Anforderungen, und drei davon entstanden aus **widerlegten** Behauptungen. **`SWR-227`**: wer die Organisationswurzel **selbst** auflöst, sagt, was er dort braucht — drei Formen statt einer (`am_bestand` für die Klasse, `methode_am_bestand` für **eine Methode**, `am_vorhandenen_bestand` mit Pflichtbegründung). ⚠⚠ Anlass: `SWR-222` prüft **deklarierte** Eingaben und kann eine **fehlende** Deklaration strukturell nicht finden — 16 Klassen lösten die Wurzel selbst auf, keine stand im Register. **`SWR-228`**: der automatische Takt nimmt keine Review-Entscheidung zurück (`auflage: offen` + Zweitlauf-Sperre, **Filter statt Veto**). **`SWR-229`** (P16): kein Kalenderschreiben ohne Bestätigung für **genau diesen** Vorgang — ⚠ ausdrücklich **vor** dem Bau geschrieben, weil das Abnahmekriterium `Z3` nicht der Text ist, sondern die **Reihenfolge**; `status: offen`, damit `trace_matrix` bei **0 Lücken** bleibt (gemessen, nicht vermutet). **`SWR-230`**: ein Befund gehört dorthin, wo der Lauf ohnehin hinsieht — die CI-Zeile im Preflight, mit dem **veralteten Grün** als schärfstem der drei Fälle. Bestand **226 → 230 SWRs, 0 Lücken**. | SWR-227…230 |
| 2026-08-22 | **Sprint 39** — sechs neue Anforderungen: **`SWR-221`/`222`/`223`** (eine Zusicherung am echten Bestand benennt ihre **eigene** Eingabe; Gegenprobe am vollen Bestand; dieselbe Bauform in JS), **`SWR-224`/`225`** (genau ein Zeitraum im Widget; ein Grund mehrerer Kacheln steht einmal), **`SWR-226`** (die Plantabelle wird an ihren Spalten erkannt, nicht an ihrer Stelle). Bestand **220 → 226 SWRs, 0 Lücken**. | SWR-221…226 |
